...
import type { Role, FormField, ApiError, WorkflowDefinition } from '@/types';
...
const fetchData = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      const workflowRes = await api.get<WorkflowDefinition>(`/admin/workflows/${route.params.id}`);
      const wf = workflowRes.data;
      name.value = wf.name;
      description.value = wf.description || '';
      fields.value = wf.form_schema.fields.map((f: any) => ({
        ...f,
        options: f.options || []
      }));
...
const saveWorkflow = async () => {
  ...
  try {
    const stepsWithOrder = steps.value.map((step, index) => ({
      ...step,
      order: index + 1
    }));
    ...
    if (isEditing.value) {
      await api.put(`/admin/workflows/${route.params.id}`, payload);
    } else {
...
<template>
  <div class="max-w-5xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-3xl font-black text-slate-900 tracking-tight leading-none mb-2">
            {{ isEditing ? 'Edit Workflow' : 'Workflow Builder' }}
          </h1>
    ...
    <div v-if="isLoading" class="p-20 flex flex-col items-center justify-center gap-6">
      ...
      <p class="text-gray-400 font-black uppercase tracking-widest text-xs">Loading workflow configuration...</p>
    </div>
